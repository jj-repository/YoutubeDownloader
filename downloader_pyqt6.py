...
QLabel#previewLabel { background: #1a1a1a; color: #d4d4d4; }
QScrollArea#urlListScroll { border: 1px solid #555555; background: #252525; }
QWidget#urlListInner { background: #252525; }
"""
...
QLabel#previewLabel { background: gray20; color: white; }
QScrollArea#urlListScroll { border: 1px solid #cccccc; background: #ffffff; }
QWidget#urlListInner { background: #ffffff; }
"""
...
class YouTubeDownloader(QMainWindow):
    ...
    def __init__(self):
        ...
        # Defer old temp-dir cleanup (moved here from _init_temp_directory so trimming_mgr exists)
        QTimer.singleShot(
            0, lambda: self.thread_pool.submit(self.trimming_mgr.cleanup_old_temp_dirs)
        )

        # Clean up leftover files from previous self-updates (off GUI thread)
        self.thread_pool.submit(UpdateManager.cleanup_old_updates)

        # HW encoder and encoding setup (dependency check runs async via DownloadManager)
        ...
        self.thread_pool.submit(self.download_mgr.init_dependencies_async)
        ...
        self._filesize_fetch_seq = 0
        self._clipboard_batch_lock = threading.Lock()
        ...
        if DBUS_AVAILABLE:
            try:
                ...
                klipper_interface = dbus.Interface(klipper, "org.kde.klipper.klipper")
                self.clipboard_mgr.set_klipper_interface(klipper_interface)
        ...
        # Config persistence debounce timer
        ...
        # Download timeout monitor (replaces per-download daemon thread)
    ...
    def _setup_trimmer_ui(self, layout: QVBoxLayout):
        ...
        self.start_preview_label.setObjectName("previewLabel")
        ...
        self.end_preview_label.setObjectName("previewLabel")
    ...
    def setup_clipboard_mode_ui(self, layout: QVBoxLayout):
        ...
        self.clipboard_url_list_widget.setObjectName("urlListInner")
    ...
    def setup_uploader_ui(self, layout: QVBoxLayout):
        ...
        self.uploader_file_scroll.setObjectName("urlListScroll")
        ...
        self.uploader_file_list_widget.setObjectName("urlListInner")
    ...
    def _apply_theme(self):
        ...
        All widget-specific styles use QSS object names (#previewLabel,
        #urlListScroll, #urlListInner) so a single setStyleSheet call suffices.
        """
        ...
        try:
            if CLIPBOARD_URLS_FILE.exists():
                with open(CLIPBOARD_URLS_FILE, "r", encoding="utf-8") as f:
                    ...
                    self.persisted_clipboard_urls = data["urls"][:MAX_CLIPBOARD_URLS]
                    logger.info(
    ...
    def _save_clipboard_urls(self):
        ...
        Snapshots data on calling thread, writes to disk off GUI thread.
        During shutdown, writes synchronously (thread pool may be shut down).
        """
        try:
            ...
            if self._shutting_down:
                self._write_clipboard_urls_to_disk(urls_to_save)
            else:
                self.thread_pool.submit(self._write_clipboard_urls_to_disk, urls_to_save)
        except Exception as e:
            logger.error(f"Error saving clipboard URLs: {e}")

    @staticmethod
    def _write_clipboard_urls_to_disk(urls: list):
        """Write clipboard URL data to disk atomically. Called from worker thread."""
        try:
            ...
            logger.info(f"Saved {len(urls)} clipboard URLs")
        except Exception as e:
            logger.error(f"Error writing clipboard URLs: {e}")
    ...
    def _restore_clipboard_urls(self):
        ...
        if hasattr(self, "persisted_clipboard_urls") and self.persisted_clipboard_urls:
            for url_data in self.persisted_clipboard_urls:
                ...
                # Re-validate URLs on restore (defense-in-depth)
                is_valid, _ = utils.validate_youtube_url(url)
                if not is_valid:
                    logger.warning(f"Skipping invalid persisted URL: {url[:80]}")
                    continue
    ...
    def _init_temp_directory(self):
        """Initialize temp directory. Cleanup of orphaned dirs is scheduled separately
        after trimming_mgr is created (see __init__)."""
    ...
    def _do_run_on_gui(self, fn):
        ...
        try:
            ...
            logger.error(f"_do_run_on_gui callback raised: {e}")
    ...
    def _poll_clipboard(self):
        ...
        try:
            clipboard_content = self.clipboard_mgr.read_clipboard_content()
    ...
    def _add_url_to_clipboard_list(self, url):
        ...
        with self.clipboard_mgr.clipboard_lock:
            ...
            if len(self.clipboard_mgr.clipboard_url_list) >= MAX_CLIPBOARD_URLS:
    ...
    def clear_all_clipboard_urls(self):
        ...
        with self.clipboard_mgr.clipboard_lock:
            ...
            self.clipboard_mgr.completed_count = 0
    ...
    def _update_url_status(self, url, status):
        ...
        completed (green), failed (red). Also maintains status counters."""
        item = self.clipboard_url_widgets.get(url)
        if item is None:
            return
        ...
        if status_label:
            status_label.setStyleSheet(f"background: {color}; border-radius: 6px; border: none;")
        ...
        with self.clipboard_mgr.clipboard_lock:
            ...
            if old_status != status:
                ...
                if old_status == "completed":
                    self.clipboard_mgr.completed_count -= 1
                elif old_status == "failed":
                    self.clipboard_mgr.failed_count -= 1
                elif old_status == "downloading":
                    self.clipboard_mgr.downloading_count -= 1
                if status == "completed":
                    self.clipboard_mgr.completed_count += 1
                elif status == "failed":
                    self.clipboard_mgr.failed_count += 1
    ...
    def _snapshot_clipboard_state(self) -> dict:
        """Capture clipboard widget values on GUI thread (thread-safe snapshot)."""
        return {
    ...
    def start_clipboard_downloads(self):
        ...
        clip_state = self._snapshot_clipboard_state()
    ...
    def _process_clipboard_queue_batch(self, pending_urls, clip_state, total_count):
        ...
        try:
            ...
            with self._clipboard_batch_lock:
            ...
            for line in process.stdout:
                ...
                # Parse progress (throttled to avoid flooding GUI event queue)
                if "[download]" in line or "Downloading" in line:
                    progress_match = PROGRESS_REGEX.search(line)
                    if progress_match:
                        now = time.time()
                        if now - _last_progress_emit > 0.25:
    ...
    def _download_clipboard_url(
        ...
        try:
            ...
            audio_only = quality.startswith("none") or clip_state.get("audio_only", False)
            url_is_playlist = utils.is_playlist_url(url)
            full_playlist_enabled = clip_state["full_playlist"]

            download_as_playlist = url_is_playlist and full_playlist_enabled
            ...
            for line in process.stdout:
                ...
                if "[download]" in line or "Downloading" in line:
                    ...
                    if progress_match:
                        progress = float(progress_match.group(1))
                        now = time.time()
                        if now - _last_seq_progress_emit > 0.25:
                            _last_seq_progress_emit = now
                            self._safe_after(
                                0, lambda p=progress: self.update_clipboard_progress(p)
                            )
                            phase = current_phase
    ...
    def _finish_clipboard_downloads(self):
        ...
        with self.clipboard_mgr.clipboard_lock:
            has_urls = len(self.clipboard_mgr.clipboard_url_list) > 0
            completed = self.clipboard_mgr.completed_count
    ...
    def stop_clipboard_downloads(self):
        ...
        with self._clipboard_batch_lock:
            proc = self._clipboard_batch_process
            self._clipboard_batch_process = None
        if proc:
            utils.safe_process_cleanup(proc)
        if was_downloading:
    ...
    def _auto_download_single_url(self, url):
        ...
        with self.clipboard_mgr.auto_download_lock:
            with self.clipboard_mgr.clipboard_lock:
                downloading_count = self.clipboard_mgr.downloading_count
    ...
    def _update_auto_download_total(self):
        ...
        with self.clipboard_mgr.clipboard_lock:
            ...
            completed = self.clipboard_mgr.completed_count + self.clipboard_mgr.failed_count
    ...
    def _fetch_file_size(self, url, quality=None, audio_only=None):
        ...
        self._filesize_fetch_seq += 1
        my_seq = self._filesize_fetch_seq
        ...
        def _fetch():
            try:
                ...
                if result.returncode == 0:
                    ...
                    if self._filesize_fetch_seq != my_seq:
                        return  # stale fetch, newer one in flight
                    if filesize:
    ...
    def on_start_slider_change(self, _value):
        """Handle start slider value changed."""
        self.on_slider_change()

    def on_end_slider_change(self, _value):
    ...
    def _on_uploader_queue_done(self, count, completed_normally):
    ...
    def start_download(self):
        ...
        if not self.download_mgr.dependencies_ok:
            self.sig_show_messagebox.emit(
                "error",
                "Error",
        ...
        # Submit download to thread pool; timeout monitor via QTimer (no thread needed)
        self.thread_pool.submit(self.download_mgr.download, url, ui_state)
        self._download_timeout_timer.start()

    def _check_download_timeout(self):
        """Check download for timeouts (called by QTimer on GUI thread)."""
        if not self.download_mgr.is_downloading or self._shutting_down:
            self._download_timeout_timer.stop()
            return
        self.download_mgr._monitor_download_timeout_tick()
    ...
    def view_upload_history(self):
        ...
        # Load and display history (offloaded to avoid blocking GUI thread)
        text_edit.setPlainText("Loading history...")
            ...
            try:
                if UPLOAD_HISTORY_FILE.exists():
                    with open(UPLOAD_HISTORY_FILE, "r", encoding="utf-8") as f:
                        content = f.read()
                    return content if content else "No upload history yet."
                return "No upload history yet."
            except Exception as e:
                return f"Error loading history: {e}"

        def _on_history_loaded(future):
            try:
                text_edit.setPlainText(future.result())
            except Exception:
                text_edit.setPlainText("Error loading history.")

        future = self.thread_pool.submit(_load_history)
        future.add_done_callback(lambda f: self._safe_after(0, lambda: _on_history_loaded(f)))
